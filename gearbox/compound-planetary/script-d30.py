...
out = revolution(wire([
	...
	bearing_center - (bearing_height*0.7 + dscrew_out)*Z + (out_int.radius - dscrew_out)*X,
...
holes, bolts_out_int = circular_screwing(out_int.axis, 
	...
	diameters=1, hold=1.3*dscrew_out)
...
for i, phase in enumerate(planets_phases):
	...
	specific_planet = planet.transform(t).own(faces=True, groups=True, tracks=True)
	#specific_planet.replace(blendpair(specific_planet.group(4).outlines(), tangents='straight'), [4])
	l1, l2 = specific_planet.group(4).outlines().arcs()
	specific_planet.replace(blendpair(l1, blending.synchronize(l1, l2), tangents='straight'), [4])
	specific_planet.mergeclose()
	...
	label = extrusion(
			...
			alignment=0.5)
	labeled = intersection(specific_planet, label.flip()).finish()
