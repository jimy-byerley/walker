...
def hgear(step, teeth, height, angle, **kwargs):
	...
	return helix(profile, height, angle, step*teeth/(2*pi), alignment=0.5)
...
def compound_planetary(
	...
	carrier = Circle(
		Axis(in_center, Z), 
		in_step*(zcrown_in-zplanet_in)/(2*pi))
	planet_out = Circle(
		Axis(out_center+carrier.radius*X,Z), 
		out_step*zplanet_out/(2*pi))
	crown_out = Circle(
		Axis(out_center,Z), 
		carrier.radius+planet_out.radius)
	planet_in = Circle(
		Axis(in_center + carrier.radius*X,Z), 
		in_step*zplanet_in/(2*pi))
	crown_in = Circle(
		Axis(in_center,Z), 
		carrier.radius + planet_in.radius)
	sun = Circle(
		Axis(in_center,Z), carrier.radius - planet_in.radius)
	...
	input = Solid(
		...
		center = in_center + height*0.5*Z + shell_thickness*Z + 1.2*dscrew_in*Z,
		ext = Solid(),
		int = Solid(),
		)
	...
	i = 0  # BUG prevent uimadcad scoping bug
	if details:
		# generate all gears teeth
		teeth = Solid()
		hangle = radians(20)
		teeth.sun = hgear(in_step, zsun, height+2*gap, -hangle, offset=-0.1*in_step, asymetry=-0.1) .transform(rotate(pi/zsun-pi*zplanet_in/zsun, Z)) .transform(sun.center)
		teeth.crown_in = hgear(in_step, zcrown_in, height+4*gap, hangle, offset=0.1*in_step, asymetry=0.1).flip() .transform(crown_in.center)
		teeth.crown_out = hgear(out_step, zcrown_out, height+2*support_height, hangle, offset=0, asymetry=0.2).transform(crown_out.center).flip()
		# BUG weird numeric issues
	#	teeth.sun = inflate(hgear(in_step, zsun, height+2*gap, -hangle, offset=-0.1*in_step, asymetry=-0.1) .transform(rotate(pi/zsun-pi*zplanet_in/zsun, Z)) .transform(sun.center), -play)
	#	teeth.crown_in = inflate(hgear(in_step, zcrown_in, height+4*gap, hangle, offset=0.1*in_step, asymetry=0.1).flip() .transform(crown_in.center), -play)
	#	crown_out_teeth = inflate(hgear(out_step, zcrown_out, height+2*support_height, hangle, offset=0, asymetry=0.2).transform(crown_out.center).flip(), -play)
		teeth.planet_in = hgear(in_step, zplanet_in, height+2*gap, hangle, offset=0.1*in_step, asymetry=0.0).transform(planet_in.center) .option(color=color_gear)
		teeth.planet_out = hgear(out_step, zplanet_out, height+2*gap, hangle, offset=0, asymetry=0.2).transform(planet_out.center) .option(color=color_gear)
		
		# shape the teeth side
		teeth.crown_out = union(teeth.crown_out, revolution(wire([
			- out_step*0.6*X - 2*gap*Z,
			+ out_step*0.6*X,
			]) .transform(out_center - height*0.5*Z + crown_out.radius*X)))
		teeth.crown_out = intersection(teeth.crown_out, revolution(wire([
			- out_step*0.8*X - gap*Z,
			+ out_step*0.8*X + gap*Z,
			]) .transform(out_center + height*0.5*Z + crown_out.radius*X)))
	
		teeth.crown_in = intersection(teeth.crown_in, revolution(wire([
			+ in_step*0.8*X - gap*Z,
			- in_step*0.6*X + gap*Z,
			]) .transform(in_center - height*0.5*Z + crown_in.radius*X)))
		teeth.crown_in = union(teeth.crown_in, revolution(wire([
			+ in_step*0.6*X - gap*Z,
			- in_step*0.8*X + 2*gap*Z,
			]) .transform(in_center + height*0.5*Z + crown_in.radius*X)))
	
		teeth.sun = union(teeth.sun, revolution(wire([
			+ in_step*0.8*X +2*gap*Z,
			- in_step*0.6*X -gap*Z,
			]) .transform(in_center + height*0.5*Z + sun.radius*X)))
		teeth.sun = intersection(teeth.sun, revolution(wire([
			+ in_step*0.6*X +gap*Z,
			- in_step*0.6*X -gap*Z,
			]) .transform(in_center - height*0.5*Z + sun.radius*X)))
		...
		planets = [(
					teeth.planet_in.transform(rotatearound(-phase*zcrown_in/zplanet_in, planet_in.axis))
					+ teeth.planet_out.transform(rotatearound(-phase*zcrown_out/zplanet_out, planet_out.axis))
		...
		planet = intersection(planet_body, teeth.planet_in + teeth.planet_out).option(color=color_gear)

	else:
		planets_phases = [0]
		planet_support = revolution(wire([
			-(support_height+0.5*height-gap)*Z + hole*X,
			+(support_height+0.5*height-gap)*Z + hole*X,
			]).transform(crown_out.center))
	...
	input_body = intersection(input_body, in_coupling)
	if details:
		input_body = intersection(input_body, teeth.sun)
	...
	if details:
		output_body = intersection(
			...
			holes.output.int + teeth.crown_out)
	else:
		output_body = intersection(
	...
	shell_outside = wire([
		...
		in_center + (input.ext.perimeter.radius + 1.5*dscrew_in)*X,
		]).segmented().flip()
	filet(shell_outside, [6, 5], width=0.5*height)
	filet(shell_outside, [4], width=0.5*dscrew_out)
	
	if details:
		shell_inside = wire([
			crown_in.center - height*0.5*Z - 0.3*support_height*Z + crown_in.radius*X,
			
			crown_in.center - height*0.5*Z - 0.3*support_height*Z + (crown_out.radius + out_step*0.6 + 2*shell_thickness)*X,
		...
		filet(shell_inside, [1], width=0.5*height)
		
		shell = revolution(shell_outside + shell_inside)
		shell = intersection(shell, teeth.crown_in)
	else:
	...
	content = {
		'in': input,
		'out': Solid(
			bolts = bolts.output.int[1:],
			**output.int,
			),
		'ground': Solid(
			support = planets_support,
			shell = shell,
			bearing = bearing,
			bolts = bolts.output.ext[1:] + bolts.input.ext[1:],
			ouput = output.ext,
			input = input.ext,
			),
		}
	joints = [
		Revolute(('in', 'out'), axis),
		Revolute(('in', 'ground'), axis),
		]
	for i, phase in enumerate(planets_phases):
		p = rotate(phase,Z) * planet_in.center
		joints.extend([
			Gear(('in', 'planet_{}'.format(i)), -zsun/zplanet_in, p, axis, Axis(p,Z)),
			Gear(('ground', 'planet_{}'.format(i)), zcrown_in/zplanet_in, p, axis, Axis(p,Z)),
			Gear(('out', 'planet_{}'.format(i)), zcrown_out/zplanet_out, p, axis, Axis(p,Z)),
			])
		if details:
			content['planet_{}'.format(i)] = planets[i]
	kin = Kinematic(joints, ground='ground', content=content)
...
if __name__ == '__madcad__':
	...
	#	bolts.input.ext.perimeter_screw = bolt(
	#		input.ext.perimeter.center + input.ext.perimeter.radius*X,
	#		input.ext.perimeter.center + input.ext.perimeter.radius*X + 1*dscrew_in*Z,
	#		dscrew_in,
	#		nutb=False)
	...
	#io.write(output.int.perimeter, '/tmp/compound-planetary/output.int.perimeter.stl')
