...
joints = []
places = {
	'front-right': Axis(h*X+w*Y, normalize(6*X-Y)),
	'back-right': Axis(-h*X+w*Y, normalize(-6*X+Y)),
	'front-left': Axis(h*X-w*Y, normalize(6*X+Y)),
	'back-left': Axis(-h*X-w*Y, normalize(-6*X-Y)),
	}
for name, axis in places.items():
	joints.extend([
		Revolute(('chest', name+'-shoulder'), axis.transform(-a*axis.direction), Axis(-a*X, X)),
